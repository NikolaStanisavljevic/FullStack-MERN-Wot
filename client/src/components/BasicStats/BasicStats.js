...
import { setStats } from '../../actions/setActions';
...
class basicStats extends Component {
  ...
  render() {
    ...
    // Loop through API Object and map it as jsx elements
    const stats = Object.keys(info).map((key, index) => {
      const myStats = info[key];

      // Make data calculations for table
...
export default connect(mapStateToProps, { getStats, setStats } )(basicStats);
